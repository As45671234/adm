...
export function getAdminToken() {
  return localStorage.getItem('adm_admin_token') || '';
}

export function setAdminToken(token: string) {
  localStorage.setItem('adm_admin_token', token);
}

export function clearAdminToken() {
  localStorage.removeItem('adm_admin_token');
}
...
export async function fetchSiteHomeContent() {
  return request<{ content: any }>('/api/site/home');
}
...
export async function adminImportExcel(token: string, file: File, categoryId?: string, categoryTitle?: string) {
  const fd = new FormData();
  fd.append('file', file);
  if (categoryId) fd.append('categoryId', categoryId);
  if (categoryTitle) fd.append('categoryTitle', categoryTitle);
...
export async function adminFetchSiteHomeContent(token: string) {
  return request<{ content: any }>('/api/admin/site/home', {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export async function adminPatchSiteHomeContent(token: string, content: any) {
  return request<{ ok: boolean; content: any }>('/api/admin/site/home', {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${token}` },
...
export async function adminCreateCategory(token: string, body: { title: string; image?: string }) {
